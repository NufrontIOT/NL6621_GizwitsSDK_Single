...
int mqtt_connect(mqtt_broker_handle_t* broker)
{
  ...
  int ret;
    ...
    fixed_header = (uint8_t*)mem_malloc( fixedHeaderSize );
    if( fixed_header==NULL )
    {
        mem_free(fixed_header);
        return -1;
    }
	...
	packet = (uint8_t*)mem_malloc( fixed_headerLen+sizeof(var_header)+payload_len );
    if( packet==NULL )
    {
        mem_free(fixed_header);
        mem_free(packet);
	...
	memcpy((unsigned char*)(packet + offset), (unsigned char*)broker->clientid, clientidlen);
	offset += clientidlen;
	...
	if(usernamelen) {
		...
		memcpy((unsigned char*)(packet+offset), (unsigned char*)broker->username, usernamelen);
		offset += usernamelen;
	}
	...
	if(passwordlen) {
		...
		memcpy((unsigned char*)(packet+offset), (unsigned char*)broker->password, passwordlen);
		offset += passwordlen;
	}	
...
int mqtt_ping(mqtt_broker_handle_t* broker) {
	...
	log_notice("\tSend MQTT heart beat data success.\n");
...
int mqtt_publish_with_qos(mqtt_broker_handle_t* broker, const char* topic, const char* msg, uint8_t retain, uint8_t qos, uint16_t* message_id) {
   ...
   if (remainLen <= 127) {
	...
	/**********************add by alex******************************/	
	 packet = ( uint8_t* )mem_malloc( fixed_headerLen+var_headerLen+msglen );
	 packetLen = fixed_headerLen+var_headerLen+msglen;	
	 
	memset(packet, 0, packetLen);
	memcpy(packet, fixed_header, fixed_headerLen);
	memcpy(packet+fixed_headerLen, var_header, var_headerLen);
	memcpy(packet+fixed_headerLen+var_headerLen, (unsigned char*)msg, msglen);
                             ...
                             uint16_t* message_id) 
{
	...
	var_header[0] = topiclen>>8;
	var_header[1] = topiclen&0xFF;
	memcpy(var_header+2, (unsigned char*)topic, topiclen);
	if(qos_size) {
		var_header[topiclen+2] = broker->seq>>8;
		var_header[topiclen+3] = broker->seq&0xFF;
		if(message_id) { // Returning message id
...
int mqtt_subscribe(mqtt_broker_handle_t* broker, const char* topic, uint16_t* message_id) {
	...
	utf_topic = ( uint8_t*)mem_malloc( topiclen+3 );
    if( utf_topic==NULL )
    {
        mem_free(utf_topic);
        return -1;
	...
	memcpy(utf_topic+2, (unsigned char*)topic, topiclen);
	/*********************************************************/
	...
	packet = (uint8_t*)mem_malloc( packetLen );
	if( packet==NULL )
    {
        mem_free(utf_topic);
        mem_free(packet);
...
int mqtt_unsubscribe(mqtt_broker_handle_t* broker, const char* topic, uint16_t* message_id) {
	...
	/******************add by alex**********************************/
	// utf topic	
	utf_topic = (uint8_t*)mem_malloc( topiclen+2 );
    if( utf_topic==NULL )
    {
        mem_free( utf_topic );
        return -1;
