...
int GAgent_GetMCUInfo( u32 Time )
{
    ...
    memcpy( g_Xpg_GlobalVar.Xpg_Mcu.product_key, "da9471bea92540da94b4e3fd14f63c68", 32 );
...
#ifdef DEV_LIGHT_MASK
	log_info("Report data: Switch:%d, RED:%d, GREEN:%d, BLUE:%d, WHITE:%d.\n",
...
#ifdef DEV_STRIP_MASK
	log_info("\tOnOff:0x%2x, week repeat:%d, timer on:%d, timer off:%d, countDown:%d, power consumption:%d.\n\n",
...
int GAgentV4_CtlDev_with_p0(int fd, unsigned char cmd, unsigned char *data, unsigned short data_len)
{
    ...
    if (data_len > (4 * 1024))
    {
		...
		if (data[1] && 0xff != 0x0) {
			/* Control Device */
			log_info("\nStart to control Device.action:0x%02x, attr_flags:0x%02x.\n", data[0], data[1]);
