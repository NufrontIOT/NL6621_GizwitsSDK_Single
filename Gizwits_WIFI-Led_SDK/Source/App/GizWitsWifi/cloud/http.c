...
int Http_InitSocket( int flag )
{
    ...
    {
        log_info("Close HTTP SocketID:[%d]\n", g_Xpg_GlobalVar.http_socketid);
    ...
    {
        log_err(" Http socket init fail\n");
        return 1;
    }    
    
    log_info("New Http socketID [%d]\n", iSocketId);        

    ret = connect_mqtt_socket(iSocketId, &socket_address, port, HttpServerIpAddr);
    if (ret < 0) {
        GAgent_Printf(GAGENT_WARNING, "http socket connect fail with:%d\n", ret);
        close(iSocketId);
        return 3;
    }
...
int Http_Response_DID( char *Http_recevieBuf, char *DID )
{
    ...
    memcpy( (unsigned char*)DID, (unsigned char*)p_start, GAGENT_DID_LEN);
