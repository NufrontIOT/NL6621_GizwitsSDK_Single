...
int32 GAgent_CreateTcpServer( uint16 tcp_port )
{
    ...
    if(listen( serversocketid, LAN_TCPCLIENT_MAX ) != 0 )
    {
	 ...
	 GAgent_Printf(GAGENT_DEBUG,"TCP Server socketid:%d on port:%d", serversocketid, tcp_port);
...
int32 GAgent_CreateUDPBroadCastServer( uint16 udpbroadcast_port, struct sockaddr_t *sockaddr)
{
    ...
    if( setsockopt(serversocketid, SOL_SOCKET, SO_BROADCAST, (char *)&opt,sizeof(int)) != 0 )
    {
...
int32 Http_ResGetFirmware( pgcontext pgc,int32 socketid )
{
    ...
    //     return RET_FAILED;
    // }
    Http_GetMD5( httpReceiveBuf,(unsigned char*)MD5,pgc->mcu.MD5 );
    ...
    if(memcmp((unsigned char*)MD5, md5_calc, 16) != 0)
    {
