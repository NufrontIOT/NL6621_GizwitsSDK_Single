...
uint32 trans_parsemcuotainfo(pgcontext pgc, ppacket pp)
{
    ...
    p = pp->ppayload + 1;
    memcpy(pgc->tmcu.pk, (INT8U *)p, 32);
    p += 32;
    memcpy(pgc->tmcu.did, (INT8U *)p, 32);
    p += 32;
    memcpy(pgc->tmcu.hv, (INT8U *)p, 8);
    p += 8;
    memcpy(pgc->tmcu.sv, (INT8U *)p, 8);
    p += 8;
    pgc->tmcu.check = *p;
...
int8 *trans_parseotaurl(ppacket pp, u8 *url)
{
    ...
    p = pp->ppayload + 1;
    memcpy(&len, (INT8U *)p, sizeof(len));
    len = htons(len);
    p += sizeof(len);
    memcpy(url, (INT8U *)p, len);
...
{
    int i = 0;
    if(RET_SUCCESS == GAgent_MCUOTAByUrl(pgc, (int8 *)url))
    {
        /* 下载固件成功 */
