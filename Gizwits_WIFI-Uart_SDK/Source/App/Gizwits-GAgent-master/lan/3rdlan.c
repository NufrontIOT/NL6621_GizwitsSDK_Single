...
void GAgent_JD_Discover_Ack( int32 udp_socket,struct sockaddr_t *addr,uint8 *szMac,int8 *uuid,int8 *feed_id )
{
    ...
    memcpy((unsigned char*)(buf_ack+pos),(unsigned char*)mac,macLen);
    pos+=macLen;
    memcpy((unsigned char*)(buf_ack+pos),(unsigned char*)productuuid,productuuidLen);
    pos+=productuuidLen;
    memcpy( (unsigned char*)(buf_ack+pos),(unsigned char*)Temp_feed_id,feedidLen);
    pos+=feedidLen;
...
int32 GAgent_JD_Get_Feedid_Key( GAgent3Cloud  *gagent3rdcloud,int8 *buf )
{
    ...
    int8 *p_end=NULL;
    int8 isChange=0;
    p_start = GAgent_strstr( buf+19,"\"accesskey\"");
    if(p_start==NULL)
    {
    ...
    if( 0!=memcmp( (unsigned char*)gagent3rdcloud->jdinfo.access_key,(unsigned char*)p_start,(p_end-p_start) ) )
    {
        isChange=1;
        memcpy( (unsigned char*)gagent3rdcloud->jdinfo.access_key,(unsigned char*)p_start,(p_end-p_start) );
    ...
    p_start = GAgent_strstr(p_end,"\"feedid\"");
    if(p_start==NULL)
    {
        return RET_FAILED;
    }
    ...
    if( 0!=memcmp((unsigned char*)gagent3rdcloud->jdinfo.feed_id,(unsigned char*)p_start,(p_end-p_start)) )
    {
        isChange=1;
        memcpy((unsigned char*)gagent3rdcloud->jdinfo.feed_id,(unsigned char*)p_start,(p_end-p_start));
...
void GAgent_JD_Write_Ack( int32 udpsocket,struct sockaddr_t *addr,int32 flag )
{
    ...
    if( buf_ack==NULL )   return ;
    pos+=JD2HEADER_LEN;
    memcpy((unsigned char*)(buf_ack+pos),(unsigned char*)data_ok,dataLen );
    if(flag>=0)
    {
        memcpy((unsigned char*)(buf_ack+pos),(unsigned char*)data_ok,dataLen );
    ...
    {
        memcpy((unsigned char*)(buf_ack+pos),(unsigned char*)data_no,dataLen );
