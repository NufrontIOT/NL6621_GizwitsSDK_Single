...
int32 handleWebConfig( pgcontext pgc,int32 fd)
{
    ...
    else
    {
        ...
        if(index_ssid && index_pass)
        {
            ...
            memcpy((unsigned char*)pConfigData->wifi_ssid, (unsigned char*)index_ssid, strlen(index_ssid));
            memcpy((unsigned char*)pConfigData->wifi_key, (unsigned char*)index_pass, strlen(index_pass));
