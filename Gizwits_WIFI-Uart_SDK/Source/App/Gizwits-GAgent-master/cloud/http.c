...
int32 Http_Response_Code( uint8 *Http_recevieBuf )
{
    ...
    p_start = GAgent_strstr( (char *)Http_recevieBuf," " );
    if(NULL == p_start)
    {
        return RET_FAILED;
    }
    ...
    if(p_end)
    {
        ...
        memcpy( (unsigned char*)re_code,(unsigned char*)p_start,(p_end-p_start) );
    }
...
int32 Http_Response_DID( uint8 *Http_recevieBuf,int8 *DID )
{
    ...
    p_start = GAgent_strstr( (char *)Http_recevieBuf,"did=");
    if( p_start==NULL )
        return 1;
    p_start = p_start+strlen("did=");
    memcpy((unsigned char*)DID,(unsigned char*)p_start,DID_LEN);
    DID[DID_LEN - 2] ='\0';             
    return 0;    
}
int32 Http_getdomain_port( uint8 *Http_recevieBuf,int8 *domain,int32 *port )
{
    ...
    int8 Temp_port[10]={0};
    memset( domain,0,100 );
    p_start = GAgent_strstr( (char *)Http_recevieBuf,"host=");
    if( p_start==NULL ) return 1;
    p_start = p_start+strlen("host=");
    p_end = GAgent_strstr(p_start,"&");
    if( p_end==NULL )   return 1;
    memcpy( (unsigned char*)domain,(unsigned char*)p_start,( p_end-p_start ));
    domain[p_end-p_start] = '\0';
    p_start = GAgent_strstr((++p_end),"port=");
    if( p_start==NULL ) return 1;
    p_start =p_start+strlen("port=");
    p_end = GAgent_strstr( p_start,"&" ); 
    memcpy((unsigned char*)Temp_port,(unsigned char*)p_start,( p_end-p_start));
    *port = atoi(Temp_port);
    return 0;
}
/******************************************************
...
int32 Http_GetSoftver_Url( int8 *download_url, int8 *softver, uint8 *buf )
{
    ...
    p_start = GAgent_strstr( (char *)buf,"soft_ver=");
    if( p_start==NULL )  return 1;
    p_start =  p_start+(sizeof( "soft_ver=" )-1);
    p_end = GAgent_strstr( p_start,"&");
    if( p_end==NULL )  return 1;
    memcpy( (unsigned char*)softver,(unsigned char*)p_start,(p_end-p_start));

    p_start = GAgent_strstr(p_end,"download_url=");
    if(p_start==NULL ) return 1;
    p_start+=sizeof("download_url=")-1;
    p_end = GAgent_strstr(p_start,kCRLFNewLine);
    if( p_end==NULL )  return 1;
    memcpy( (unsigned char*)download_url,(unsigned char*)p_start,(p_end-p_start));
...
int32 Http_HeadLen( uint8 *httpbuf )
{
   ...
   p_end = GAgent_strstr( (char *)httpbuf,kCRLFLineEnding);
   if( p_end==NULL )
   {
...
int32 Http_BodyLen( uint8 *httpbuf )
{
   ...
   p_start = GAgent_strstr( (char *)httpbuf,"Content-Length: ");
   if( p_start==NULL ) return 0;
   p_start = p_start+strlen("Content-Length: ");
   p_end = GAgent_strstr( p_start,kCRLFNewLine);
   if( p_end==NULL )   return 0;

   memcpy( (unsigned char*)bodyLenbuf,(unsigned char*)p_start,(p_end-p_start));
...
int32 Http_GetHost( int8 *downloadurl,int8 **host,int8 **url )
{
   ...
   p_start = GAgent_strstr( downloadurl,"http://" );
   if(p_start==NULL) 
       return RET_FAILED;
   ...
   *host = (char *)malloc(p_end-p_start+1);
   ...
   memcpy((unsigned char*)(&(*host)[0]),(unsigned char*)p_start,p_end-p_start);
   ...
   *url = (char *)malloc(p_end-p_start+1);
   ...
   memcpy((unsigned char*)(&(*url)[0]),(unsigned char*)p_start,p_end-p_start);
   (*url)[p_end-p_start] = '\0';
...
//Soft-Version: 
   ...
   int8 *p_end =NULL;
   int32 SV_Len=0;
   p_start = GAgent_strstr( (char *)httpbuf,"Soft-Version: " );
   if(p_start==NULL) 
       return 0;
   ...
   memcpy((unsigned char*)SV,(unsigned char*)p_start,SV_Len);
   SV[SV_Len]='\0';
   return SV_Len;
...
int32 Http_GetMD5( uint8 *httpbuf,uint8 *MD5,int8 *strMD5)
{
   ...
   int8 *str;
   int32 i=0,j=0;
   p_start = GAgent_strstr( (char *)httpbuf,"Firmware-MD5: ");
   if(p_start==NULL)
       return 1;
   ...
   memcpy( (unsigned char*)strMD5,(unsigned char*)p_start,32 );
   strMD5[32] = '\0';
   memcpy( MD5,(unsigned char*)p_start,16);
   MD5[16]= '\0';
   ...
   for(i=0;i<32;i=i+2)
   {
       ...
       Temp[1] = strMD5[i+1];
       Temp[2] = '\0';
       MD5_TMP[j]= strtol((const char*)Temp, &str,16);  
       j++;
   }
   memcpy(MD5,(unsigned char*)MD5_TMP,16);
...
uint8 Http_Get3rdCloudInfo( int8 *szCloud3Name,int8 *szCloud3Info,uint8 *buf )
{
    ...
    p_start = GAgent_strstr( (char *)buf,cloudName );
    if( p_start==NULL )
        return 0;
    ...
    if( p_end==NULL )
        return 0;
    memcpy( (unsigned char*)szCloud3Name,(unsigned char*)p_start,(p_end-p_start) );
    szCloud3Name[p_end-p_start]='\0';
    
    p_start = GAgent_strstr( p_start,uuid );
    if( p_start==NULL )
        return 0;
    p_start +=strlen( uuid );
    p_end = GAgent_strstr( p_start,kCRLFNewLine );
    if( p_end==NULL )
        return 0;
    memcpy( (unsigned char*)szCloud3Info,(unsigned char*)p_start,(p_end-p_start));
