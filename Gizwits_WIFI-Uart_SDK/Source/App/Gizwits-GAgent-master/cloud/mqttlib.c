...
int mqtt_connect(mqtt_broker_handle_t* broker)
{
    ...
    memcpy(packet+offset, (unsigned char*)broker->clientid, clientidlen);
    offset += clientidlen;
    ...
    if(usernamelen) {
        ...
        memcpy(packet+offset,(unsigned char*) broker->username, usernamelen);
        offset += usernamelen;
    }
    ...
    if(passwordlen) {
        ...
        memcpy(packet+offset, (unsigned char*)broker->password, passwordlen);
        offset += passwordlen;
    }	
...
/******************************************************************/
    ...
    /*     fixed_header[2] = remainLen / 128; */
    /* } */
    memcpy(fixed_header + 1, (INT8U *)sendvarc.var, sendvarc.varcbty);
    /**********************add by alex******************************/
                              ...
                              uint16_t *message_id)
{
    ...
    uint16_t topiclen = strlen((const char *)pp->phead);
/************************add by alex****************************/
    int var_headerLen;
    int fixed_headerLen;
    int packetLen;
