...
static int32 init_subTopic( mqtt_broker_handle_t* broker ,pgcontext pgc,char *Sub_TopicBuf,int32 Mqtt_Subflag)
{
    ...
    case 1:
        ...
        memcpy( (unsigned char*)Sub_TopicBuf+strlen("ser2cli_noti/"),pgc->mcu.product_key,productKeyLen );
        Sub_TopicBuf[strlen("ser2cli_noti/")+productKeyLen] = '\0';
        break;
    case 2:
        // 4.7 4.9
        memcpy( (unsigned char*)Sub_TopicBuf,"ser2cli_res/",strlen("ser2cli_res/") );
        memcpy( (unsigned char*)Sub_TopicBuf+strlen("ser2cli_res/"),(unsigned char*)pgc->gc.DID,DIdLen );
        Sub_TopicBuf[strlen("ser2cli_res/")+DIdLen] = '\0';
        break;
    case 3:
        // 4.13
        memcpy((unsigned char*)Sub_TopicBuf,"app2dev/",strlen("app2dev/") );
        memcpy((unsigned char*)Sub_TopicBuf+strlen("app2dev/"),(unsigned char*)pgc->gc.DID,DIdLen );
...
int32 MQTT__SenData( pgcontext pgc, int8 *szDID, ppacket pbuf,/*uint8 *buf,*/ int32 buflen )
{
    ...
    memcpy( (INT8U *)msgtopic,"dev2app/",strlen("dev2app/"));
    pos += strlen("dev2app/");
    memcpy( (unsigned char*)(msgtopic+strlen("dev2app/")),(unsigned char*)szDID,didlen );
    pos += didlen;
    if(0x0094 == cmd)
    {
        ...
        if(clienIdLen > 0)
        {
            msgtopic[pos] = '/';
            pos++;
            memcpy( (unsigned char*)(msgtopic+pos),(unsigned char*)clienid,clienIdLen );
            pos+=clienIdLen;
        }
    }
    msgtopic[pos] = '\0';
...
int32 MQTT_SendData( pgcontext pgc, int8 *szDID, ppacket pbuf, int32 buflen )
{
    ...
    *(uint16 *)&sendpack[4+sendvarc.varcbty+1] = htons(cmd);
    ...
    memcpy( (INT8U *)msgtopic,"dev2app/",strlen("dev2app/"));
    pos += strlen("dev2app/");
    memcpy( (INT8U *)msgtopic+strlen("dev2app/"),(INT8U *)szDID,didlen );
    pos += didlen;
    if(0x0094 == cmd)
    {
        clienIdLen = strlen( (const int8 *)clienid);
        if(clienIdLen > 0)
        {
            msgtopic[pos] = '/';
            pos++;
            memcpy( (INT8U *)msgtopic+pos,(INT8U *)clienid,clienIdLen );
            pos+=clienIdLen;
        }
    }
    msgtopic[pos] = '\0';
    sendpack -= pos;
    memcpy(sendpack, (INT8U *)msgtopic, pos);
    GAgent_Printf(GAGENT_DEBUG,"------SEND TO Cloud ------\r\n");
    /* for(i=0;i<sendpacklen;i++) */
    /*     GAgent_Printf(GAGENT_DUMP," %02X",sendpack[i] ); */
    /* 添加头部字段的长度。当前值为payload的总长度 */
...
void Mqtt_ReqOnlineClient(pgcontext pgc)
{
    ...
    memcpy(p, (INT8U *)req_buf, sizeof(req_buf));
    p += sizeof(req_buf);
    pp->pend = p;
