...
uint32 GAgent_Get_Gserver_Time( uint32 *clock, uint8 *Http_recevieBuf, int32 respondCode )
{
    ...
    p_start = GAgent_strstr((char *)Http_recevieBuf, "server_ts=");
    if( p_start==NULL ) 
        return (uint32)RET_FAILED;   
    ...
    if( p_end == NULL )
    {
        p_end = GAgent_strstr( p_start,"\r" ); 
    }    
    memcpy(stime,(INT8U *)p_start,( p_end-p_start));
    time = atoi((const char *)stime);
    *clock = time;
    return (uint32)RET_SUCCESS;
}
/****************************************************************
...
uint32 cloud_querymcuota(pgcontext pgc)
{
    ...
    http_querymcuota(socket, (int8 *)pgc->rtinfo.Txbuf->phead, HTTP_SERVER, (int8 *)pgc->tmcu.pk, (int8 *)pgc->tmcu.did, (int8 *)pgc->tmcu.hv, (int8 *)pgc->tmcu.sv, 0x02);
        ...
        switch( cloudstatus )
        {
            ...
            case CLOUD_QUERY_MCU_OTA:
                ...
                else
                {
                    ...
                    Http_GetSoftver_Url((int8 *)url, (int8 *)sv, pCloudConfiRxbuf);
