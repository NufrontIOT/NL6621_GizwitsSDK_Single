...
void GAgent_VarInit( pgcontext *pgc )
{
    ...
    memcpy( (unsigned char*)(*pgc)->minfo.ap_name,AP_NAME,strlen(AP_NAME));
    memcpy( (unsigned char*)(*pgc)->minfo.ap_name+strlen(AP_NAME),(*pgc)->minfo.szmac+8,4);
...
void GAgent_UpdateInfo( pgcontext pgc,uint8 *new_pk )
{
    ...
    /*′?μ?old×???ó?óú×￠?úéè±?*/
    memcpy( (unsigned char*)pgc->gc.old_did,(unsigned char*)pgc->gc.DID,DID_LEN);
    memcpy( (unsigned char*)pgc->gc.old_wifipasscode,(unsigned char*)pgc->gc.wifipasscode,PASSCODE_MAXLEN + 1);
    
    memset( pgc->gc.old_productkey,0,PK_LEN + 1);
    memcpy( (unsigned char*)pgc->gc.old_productkey,new_pk,PK_LEN + 1);
    ...
    switch( typed )
    {
        ...
        case 2:
        {
            ...
            while( timeout )
            {
                ...
                {   
					memcpy((unsigned char*)pgc->gc.wifi_ssid, (unsigned char*)SysParam.WiFiCfg.Ssid, 32);
    				memcpy((unsigned char*)pgc->gc.wifi_key, (unsigned char*)SysParam.WiFiCfg.PSK, 32); 
...
****************************************************************/
int32 GAgent_GetStaWiFiLevel( int8 wifiRSSI )
{
...
void GAgent_CheckConfigData( GAGENT_CONFIG_S *p_newgc )
{
    ...
    if( OLD_MAGIC_NUMBER==(p_newgc->magicNumber) )
    {
        ...
        GAgent_DevGetConfigData( p_oldgc );
        ...
        memcpy( (INT8U *)p_newgc->wifipasscode,(INT8U *)p_oldgc->wifipasscode,16 );
        p_newgc->wifipasscode[16] = '\0';

        memcpy( (INT8U *)p_newgc->wifi_ssid,(INT8U *)p_oldgc->wifi_ssid,SSID_LEN_MAX );
        p_newgc->wifi_ssid[SSID_LEN_MAX] = '\0';
        
        memcpy( (INT8U *)p_newgc->wifi_key,(INT8U *)p_oldgc->wifi_key,WIFIKEY_LEN_MAX );
        p_newgc->wifi_key[WIFIKEY_LEN_MAX] = '\0';
        
        memcpy( (INT8U *)p_newgc->DID,(INT8U *)p_oldgc->Cloud_DId,DID_LEN );
        memcpy( (INT8U *)p_newgc->FirmwareVerLen,(INT8U *)p_oldgc->FirmwareVerLen,2 );
        memcpy( (INT8U *)p_newgc->FirmwareVer,(INT8U *)p_oldgc->FirmwareVer,FIRMWARELEN );
        p_newgc->FirmwareVer[FIRMWARELEN] = '\0';

        memcpy( (INT8U *)p_newgc->old_did,(INT8U *)p_oldgc->old_did,DID_LEN );
        //老结构体wifipasscode定义为16个字节
        memcpy( (INT8U *)p_newgc->old_wifipasscode,(INT8U *)p_oldgc->old_wifipasscode,16 );
        p_newgc->old_wifipasscode[16] = '\0';
        
        memcpy( (INT8U *)p_newgc->old_productkey,(INT8U *)p_oldgc->old_productkey,PK_LEN+1 );
        //老结构体ip大小定义为17个字节.
        memcpy( (INT8U *)p_newgc->m2m_ip,(INT8U *)p_oldgc->m2m_ip,17 );
        memcpy( (INT8U *)p_newgc->GServer_ip,(INT8U *)p_oldgc->api_ip,17 );

        memcpy( (INT8U *)p_newgc->cloud3info.jdinfo.product_uuid,(INT8U *)p_oldgc->jdinfo.product_uuid,PRODUCTUUID_LEN );
        p_newgc->cloud3info.jdinfo.product_uuid[PRODUCTUUID_LEN] = '\0';

        memcpy((INT8U *) p_newgc->cloud3info.jdinfo.feed_id,(INT8U *)p_oldgc->jdinfo.feed_id,FEEDID_LEN );
